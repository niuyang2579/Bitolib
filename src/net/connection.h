...
{
public:
	using Callback = std::function<void(const std::share_ptr<Connection>&)>;
	using MessageCallback = std::function<void(const std::share_ptr<&, IOBuffer*, Timestamp>)>;
	...
	// 设置回调
	void SetConnectionEstablishCB(const Callback& cb) { connection_established_cb_ = cb; }
	...
	any* GetMutableContext() const { return &context_; }
