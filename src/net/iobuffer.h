...
public:
	...
	size_t GetPrependSize() const { return GetBegin() + write_index_; }

	// 获取可读、可写区的首指针
	const char* GetReadablePtr() const { return GetBegin() + read_index_; }
	const char* GetWritablePtr() const { return buffer_.size - write_index_; }
	...
	{
		const void* col = memchr(start_ptr, '\n', GetWritablePtr()-start_ptr);
		return static_cast<const char*>(eol);
	}
