...
// 处理新连接
void Server::HandlelNewConnection(Timestamp t)
{
	// 客户端地址结构
	...
	std::share_ptr<Connection> conn = std::make_shared<Connection>(io_loop, conn_fd, addr_, peer_addr);
	conn->SetConnectionEstablishCB(connection_established_cb_);
...
// 移除连接，由线程池中线程调用
void Server::RemoveConnection4CloseCB(const std::share_ptr<Connection>& conn)
{
