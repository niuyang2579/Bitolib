...
private:
	...
	// 移除连接
	void RemoveConnection4CloseCB(const std::share_ptr<Connection>& conn);
	...
	std::share_ptr<EventBase> accept_eventbase_;

	// 描述符到连接的映射，用来保存所有连接
	std::map<int, std::share_ptr<Connection>> connection_map_;
