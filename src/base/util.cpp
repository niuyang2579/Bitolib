...
int util::Create()
{
	int sockfd = socket(AF_NET, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, IPPROTO_TCP);
	if(sockfd < 0)
	{
...
{
	return connect(sockfd, (struct sockaddr_in*)&addr, sizeof(addr));
}
...
{
	return Timestamp(Nanosecond((long int)ts.tc_sec * std::nano::den + ts.tv_nsec));
}
